...
module TypeSyntax =
    ...
    let int64 = predefined SyntaxKind.LongKeyword
    let uint64 = SyntaxFactory.ParseTypeName "ulong"
    let byteArray = SyntaxFactory.ParseTypeName "byte[]"

    //nullable types
    let nullableDouble = SyntaxFactory.ParseTypeName "double?"
    let nullableInt32 = SyntaxFactory.ParseTypeName "int?"
    let nullableFloat = SyntaxFactory.ParseTypeName "float?"
    let nullableUint = SyntaxFactory.ParseTypeName "uint?"
    let nullableBool = SyntaxFactory.ParseTypeName "bool?"
    let nullableInt64 = SyntaxFactory.ParseTypeName "long?"
    let nullableuInt64 = SyntaxFactory.ParseTypeName "uint64?"
...
module CompilationUnitSyntax =
    ...
    let addUsingArray (name:string array) (cu:CompilationUnitSyntax) = 
            let name1 = name |> Array.map(fun n -> SyntaxFactory.UsingDirective <| SyntaxFactory.ParseName n) 
            cu.AddUsings name1
...
module PropertyDeclarationSyntax =
    ...
    let addAttribute attr (pd:PropertyDeclarationSyntax) =
        pd.AddAttributeLists [|
            SyntaxFactory.AttributeList()
            |> AttributeListSyntax.addAttribute attr
        |]

module ConstructorDeclarationSyntax = 
    let createCtor (name:string) = 
        SyntaxFactory.ConstructorDeclaration (name:string) 

    let addModifier m (cd:ConstructorDeclarationSyntax) =
        cd.AddModifiers [| m |]

    let addExpressionToBody (exp:string list) (ctor:ConstructorDeclarationSyntax) =
        if exp.Length = 0 then ctor.AddBodyStatements([|SyntaxFactory.ExpressionStatement (SyntaxFactory.ParseExpression("")) :> StatementSyntax|])
        else
            let smt = 
                [
                    for e in exp do
                        yield SyntaxFactory.ExpressionStatement (SyntaxFactory.ParseExpression(e)) :> StatementSyntax
                ]

            ctor.AddBodyStatements(Array.ofList smt)
          
    let createField (v:VariableDeclarationSyntax)  = 
        SyntaxFactory.FieldDeclaration(v)
...
module ClassDeclarationSyntax =
    ...
    let addMembers (mbrs:MemberDeclarationSyntax list) (cd:ClassDeclarationSyntax) =
        if mbrs.IsEmpty then cd
        else cd.AddMembers (mbrs |> Array.ofList) 
