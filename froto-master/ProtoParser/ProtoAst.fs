...
type ProtoExtension (start:int64, max:string) = 
    member val Start = start with get
    member val End = max with get
...
type ProtoMessage (name:string, parts:(ProtoMessagePart * Object) list, isExtend:bool) =
    ...
    member x.Extensions =
        x.Parts
        |> List.filter (fun (p,_) -> match p with | Extensions -> true | _ -> false)
        |> List.map (fun (_,o) -> o :?> ProtoExtension)
...
type ProtoFile (sections:(ProtoSection * Object) list) =
    ...
    member x.Enums =
        x.Sections
        |> List.filter (fun (s,_) -> match s with | Enum -> true | _ -> false)
        |> List.map (fun (_,o) -> o :?> ProtoEnum)
